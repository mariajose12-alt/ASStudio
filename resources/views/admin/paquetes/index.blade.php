...
@section('content')
    <div class="card">
        <div class="card-header">
            ...
            <span style="font-size:12px; color:var(--text-muted);">{{ $paquetes->total() }} paquetes en total</span>
        </div>
        <div class="table-wrap">
            <table>
                ...
                @forelse($paquetes as $paquete)
                    <tr>
                        <td style="color:var(--text-muted);">{{ $paquete->id }}</td>
                        <td>
                            <strong style="color:#1a1a1a;">{{ $paquete->nombre }}</strong>
                            @if($paquete->descripcion)
                                <br><span style="font-size:12px; color:var(--text-muted);">{{ Str::limit($paquete->descripcion, 50) }}</span>
                            @endif
                        </td>
