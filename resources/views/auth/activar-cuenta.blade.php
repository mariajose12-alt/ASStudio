...
<head>
    <meta charset="UTF-8">
    <title>Activar cuenta · {{ config('app.name') }}</title>
...
<div class="card">
    ...
    <p class="subtitle">Este es el último paso para activar tu cuenta en {{ config('app.name') }}.</p>

    <div class="email-hint">{{ $usuario->email }}</div>
    ...
    <form method="POST" action="{{ route('activacion.procesar', [$usuario, $token]) }}">
        @csrf
        ...
        <button type="submit" class="btn">Activar cuenta</button>
    </form>
</div>
</body>
</html>
